...
{
	public static class MimeService
	{
		private static readonly Dictionary<string, string> _types = new Dictionary<string, string>
  {
    ...
    {"zip", "application/zip"},
		...
		{
			Shield.AgainstNullOrEmpty(fileName).Raise();
			var extension =  fileName.Contains(".") ? Path.GetExtension(fileName).Remove(0, 1) : "unknown";
			return _types[extension];
		}

		public static string GetTypeByKey(string key)
		{
			Shield.AgainstNullOrEmpty(key).Raise();
			return _types[key];
		}
